...
function expectArraysToContainSameMembers<T>(actual:T[], expected:T[]) : void{
  expect(actual.length * 2).toBe(expected.length);
...
describe('CoveyTownController', () => {
  ...
  describe('Conversation Areas', () =>{
    ...
    describe('updatePlayerLocation', () =>{
      ...
      it('should respect the conversation area reported in updatePlayer by the UserLocation\'s conversationLabel property, and not override it if the (x,y) coordinates of the UserLocation would be contained in a different conversation area [T2.1]', async ()=>{
      ...
      it('should emit onConversationUpdated when players enter or exit [T2.1]', async ()=>{
        ...
        testingTown.players.push(areaOneHolder);
        expectConversationAreas(preCreatedAreas);
